...
class TileWorker(Process):
    def __init__(
        self, queue, slide_path, tile_size, threshold, format, saved_tiles, lock
    ):
        super().__init__()
        self.queue = queue
...
def extract_patches(
    ...
    for slide_file in sorted(slide_files):
        ...
        workers_list = [
            TileWorker(
                queue, slide_file, tile_size, threshold, format, saved_tiles, lock
            )
        ...
        for level in levels:
            ...
            with lock:
                logger.info(
                    f"{saved_tiles.value}/{total_tiles} tiles saved after filtering"
                )
                saved_tiles.value = 0
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to input WSI file or directory",
    )
    parser.add_argument(
        "-o", "--output", type=str, required=True, help="Path to output directory"
    )
    parser.add_argument(
        "-l", "--levels", type=int, default=[0, 1, 2], help="Levels to extract"
    )
    parser.add_argument("-t", "--tile_size", type=int, default=224, help="Tile size")
    parser.add_argument(
        "-f", "--format", type=str, default="jpeg", help="Image format (jpeg, png)"
    )
    parser.add_argument(
        "-th",
        "--threshold",
        type=float,
        default=5,
        help="Threshold for filtering background",
    )
    parser.add_argument(
        "-w", "--workers", type=int, default=20, help="Number of worker processes"
    )
